...
import { useRepositories, useClientConfig, useBranches } from '../../hooks';
import type { SessionCreationRequest } from '../SessionCreationWidget';
...
interface NewSessionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onStartCreation: (request: SessionCreationRequest) => void;
}
...
export function NewSessionModal({
  ...
  }, [isOpen, fetchRepositories, fetchClientConfig, resetBranches]);
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      // Generate unique ID for this creation process
      const id = `create-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

      // Notify parent to start creation
      onStartCreation({
        id,
    ...
    [selectedRepo, branchStrategy, baseBranch, workBranch, existingBranch, shell, devcontainerSource, selectedTemplateId, mountClaudeSettings, onStartCreation, onClose]
  );
